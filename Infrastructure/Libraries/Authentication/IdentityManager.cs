...
namespace Infrastructure.Libraries.Authentication {
    ...
    public class IdentityManager : IIdentityManager {
        ...
        private void getUserAgent() {
            try {
                this.useragent = this.getHeaderValue("User-Agent").ToString();
            } catch { }            
        }
        ...
        //--charles ipaddress implementation

        private void getIPAddress()
        {
            string[] ipAddresses = null; // Initialize to null
            string altIPAddress = null;
            try
            {
                try
                {
                    string xForwardedFor = this.getHeaderValue("X-Forwarded-For")?.ToString();
                    if (!string.IsNullOrEmpty(xForwardedFor))
                    {
                        ipAddresses = xForwardedFor.Split(':');
                    }
                }
                catch { }

                // Check if ipAddresses is still null or empty
            ...
            // Check if ipAddresses is still null or empty
            var ipAddress = (ipAddresses != null && ipAddresses.Length > 0) ? ipAddresses[0] : altIPAddress;
            this.IPAddress = ipAddress;
        }
